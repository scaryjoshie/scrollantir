...
| [sessions/handoff-runtime-onboarding.md](sessions/handoff-runtime-onboarding.md) | Onboarding doc for the runtime/ rebuild — used at the start of session-2026-04-27. Mostly historical now. |
| [sessions/session-2026-04-27.md](sessions/session-2026-04-27.md) | Post-cutover state: runtime/ live on Hetzner, devices cut over, ingest_api shipped, dashboard rewritten to PostgREST, multiple Android tracker bugs fixed via two audit passes. Operational reference (URLs, deploy commands, token mint) + open issues + key files map. |
| [sessions/session-2026-04-28.md](sessions/session-2026-04-28.md) | Content detector hardening (shorts false-positive eliminated via bounds-check + 2-match start; ID set narrowed to Shorts-exclusive containers) and LocationWatcher → HIGH_ACCURACY. Empirical chain + DigiPaws references. |
| [sessions/handoff-derived-dashboard.md](sessions/handoff-derived-dashboard.md) | Original entry-point doc for the deriver-runtime + dashboard-timeline phase. **Superseded 2026-04-29** — see the two session docs below. |
| [sessions/plan-place-visit-v1.md](sessions/plan-place-visit-v1.md) | Audited plan for `place_visit/v1` + `travel_leg/v1`. Shipped 2026-04-29; preserved for design rationale + audit-driven fix log. |
| [sessions/session-2026-04-29-gps-noise.md](sessions/session-2026-04-29-gps-noise.md) | Phone-side GPS-noise filtering: `PRIORITY_HIGH_ACCURACY` is GPS-LED not GPS-only; WiFi-triangulated fixes with optimistic accuracy were the "spazzing" source. Three-gate filter pipeline (accuracy 50m, GPS-attestation, speed-based), `gps_attested` field added. |
| [sessions/session-2026-04-29-place-visit.md](sessions/session-2026-04-29-place-visit.md) | `place_visit/v1` + `travel_leg/v1` derivers shipped, agent_api RPC + read-path views + dashboard cutover. /today renders against real GPS data, re-deriving every 5 min over a rolling 24h window. Includes knob values, decisions made, deferred work, and the live-deployment runbook. |
| [sessions/handoff-2026-04-29.md](sessions/handoff-2026-04-29.md) | **Current onboarding doc for a fresh agent.** Reading list (in order), 60-second verifiability check, what's NOT shipped (in priority), operational runbook, don't-touch list. Read this first. |
